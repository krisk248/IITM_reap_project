...
from PyQt5.QtWidgets import (
    QApplication, QWidget, QRadioButton, QButtonGroup, QLineEdit, QPushButton,
...
class BGMWorker(QThread):
    ...
    def __init__(self, mode, input_path, bgm_file, output_path, volume, use_cuda, notif_topic):
        super().__init__()
        ...
        self.created_files = []       # list to track created files for cleanup
    ...
    def build_ffmpeg_command(self, input_file, output_file):
        ...
        command = []
        if self.use_cuda:
            # Prepend CUDA hardware acceleration and use nvenc encoder.
            command.extend(["ffmpeg", "-hwaccel", "cuda", "-i", input_file])
        else:
            command.extend(["ffmpeg", "-i", input_file])
    ...
    def cleanup(self):
        ...
        # In folder mode, remove the output parent folder if it exists
        if self.mode == "folder":
            input_folder = self.input_path
            parent_name = os.path.basename(os.path.normpath(input_folder))
            output_parent = os.path.join(self.output_path, parent_name + "Wbgm")
...
class MainWindow(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("🤴🏻Kannan's --- BGM Adder 🧙 ")
        self.worker = None
        self.init_ui()
    ...
    def init_ui(self):
        ...
        self.volume_spin.setRange(0.10, 1.00)
        self.volume_spin.setSingleStep(0.01)
        self.volume_spin.setValue(0.05)
    ...
    def start_processing(self):
        ...
        self.worker = BGMWorker(mode, input_path, bgm_file, output_path, volume, use_cuda, notif_topic)
