...
# Try to import PercentageColumn; if it fails, define our own.
try:
    ...
    class PercentageColumn(ProgressColumn):
        """A simple custom percentage column."""
        def render(self, task):
            if task.total:
                percentage = (task.completed / task.total) * 100
                return f"{percentage:05.2f}%"
            return "N/A"
...
def convert_video_file(input_file: str, output_file: str) -> None:
    ...
    # Set up the Rich progress bar with percentage column.
    progress = Progress(
        ...
        with progress:
            while True:
                ...
                if line.startswith("out_time="):
                    # The value is in the format HH:MM:SS.microseconds
