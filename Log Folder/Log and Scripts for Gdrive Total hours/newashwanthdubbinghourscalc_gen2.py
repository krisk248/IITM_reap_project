...
def get_log_filename(shared_drive_id, root_folder_name):
    """Generates a log filename based on timestamp, Shared Drive ID, and root folder name."""
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    safe_root_folder_name = root_folder_name.replace(" ", "_").replace("/", "_")
    return f"duration_log_{timestamp}_{shared_drive_id}_{safe_root_folder_name}.txt"
...
def traverse_folder(service, folder_id, shared_drive_id, folder_name, log_file):
    ...
    for file in files:
        if file['mimeType'] == 'application/vnd.google-apps.folder':  # If it's a folder, recurse
            total_duration += traverse_folder(service, file['id'], shared_drive_id, file['name'], log_file)
...
def main():
    ...
    log_file = get_log_filename(shared_drive_id, root_folder_name)
    ...
    total_duration = traverse_folder(service, root_folder_id, shared_drive_id, root_folder_name, log_file)
