...
def traverse_folder(service, folder_id, shared_drive_id):
    ...
    for file in files:
        ...
        elif file['mimeType'] == 'video/quicktime':  # Process only .mov files
            print(f"Processing video: {file['name']}")
            duration = get_video_duration(service, file['id'])
            print(f"File: {file['name']}, Duration: {duration} seconds")
...
def main():
    ...
    print(f"Calculating total duration of .mov videos in Shared Drive (ID: {shared_drive_id})...")
    total_duration = traverse_folder(service, root_folder_id, shared_drive_id)
    print(f"Total duration of all .mov videos: {total_duration} seconds")
