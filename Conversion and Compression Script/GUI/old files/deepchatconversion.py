...
# Update FFMPEG_BINARY detection for macOS
try:
    import imageio_ffmpeg
    FFMPEG_BINARY = imageio_ffmpeg.get_ffmpeg_exe()
except ImportError:
    # Default to system-installed ffmpeg on macOS
    FFMPEG_BINARY = "/opt/homebrew/bin/ffmpeg" if os.path.exists("/opt/homebrew/bin/ffmpeg") else "ffmpeg"

FFPROBE_BINARY = "/opt/homebrew/bin/ffprobe" if os.path.exists("/opt/homebrew/bin/ffprobe") else "ffprobe"
...
# =============================================================================
# To bundle this script as an exe (with all dependencies including ffmpeg/ffprobe),
# you can use PyInstaller. For example:
#
#   pyinstaller --onefile --add-binary "<path_to_ffmpeg_binary>;." --add-binary "<path_to_ffprobe_binary>;." your_script.py
#
# Ensure that the bundled binaries are correctly referenced in FFMPEG_BINARY and FFPROBE_BINARY.
